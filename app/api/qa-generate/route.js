...
// ── 성인 한국어 voice 풀 ─────────────────────────────────────────────
// ★ Neural2 (가장 자연스러운 일반 voice) 우선, Wavenet 백업.
//   Standard 모델 완전 제거 (옛 voice 라 부자연스러움).
//   pitch_offset 도 자연 범위 (-2 ~ +0.5) 로 조정 — 이전엔 -4 까지 가서 부자연스러움.
const FEMALE_VOICE_POOL = [
    { name: "ko-KR-Neural2-A", pitch_offset: 0.0,  rate: 1.0 },   // 가장 자연스러운 성인 여성
    { name: "ko-KR-Neural2-B", pitch_offset: -0.5, rate: 1.0 },   // 약간 차분한 성인 여성
    { name: "ko-KR-Wavenet-A", pitch_offset: 0.3,  rate: 1.0 },   // 백업 (Wavenet)
    { name: "ko-KR-Wavenet-B", pitch_offset: 0.0,  rate: 1.0 },
];
const MALE_VOICE_POOL = [
    { name: "ko-KR-Neural2-C", pitch_offset: -1.0, rate: 1.0 },   // 가장 자연스러운 성인 남성
    { name: "ko-KR-Wavenet-C", pitch_offset: -0.5, rate: 1.0 },   // 일반 성인 남성
    { name: "ko-KR-Wavenet-D", pitch_offset: -1.5, rate: 1.0 },   // 약간 깊은 성인 남성 (이전 -3.5 → -1.5)
];
...
6. ★★ 세 질문은 반드시 서로 다른 관점이어야 함 (동일 표현/의도 반복 금지):
   ...
   - 1개: type="application" — 한계·확장·적용 ("X 가 다른 상황에서도 적용되나요?", "한계나 예외 케이스는?")
   세 질문이 같은 청크에 과도 집중 X, 같은 단어 반복 X.
...
{
  "questions": [
    { "chunkIndex": 0, "type": "clarification", "question": "질문 내용" },
    { "chunkIndex": 3, "type": "evidence",      "question": "질문 내용" },
    { "chunkIndex": 6, "type": "application",   "question": "질문 내용" }
  ]
}`;
